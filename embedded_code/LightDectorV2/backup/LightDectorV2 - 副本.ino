...
bool strobeDetectionActive = false;
unsigned long strobeDetectionStartTime = 0;
const unsigned long strobeDetectionDuration = 5000;
...
// 频闪检测
const int STROBE_SAMPLES = 100;
int strobeData[STROBE_SAMPLES];
int strobeIndex = 0;
bool strobeDataReady = false;
...
void setup() {
  Serial.begin(9600); // 提高波特率以获得更快的日志输出
  delay(2000);
  ...
  server.on("/data", HTTP_GET, getSensorData);
  server.on("/control", HTTP_POST, handleControl);
  server.on("/download", HTTP_GET, downloadData);
  ...
  Serial.println("  - GET  /data     : 传感器数据API");
  Serial.println("  - POST /control   : 控制命令API");
  Serial.println("  - GET  /download : 数据下载");
...
void loop() {
  ...
  if (!systemPaused) {
    ...
    if (currentTime - lastUpdateTime >= updateInterval) {
      ...
      // 更新历史数据
      updateLightHistory(lightValue, currentTime);
      ...
      if (strobeDetectionActive) {
        handleStrobeDetection(lightValue, currentTime);
      }
...
// 增强的传感器数据API
void getSensorData() {
  Serial.print("[" + String(millis()) + "] 客户端请求传感器数据 - ");
...
void handleControl() {
  ...
  Serial.print("[" + String(millis()) + "] 控制命令接收 - ");
  ...
  if (command == "set_rate") {
    ...
    if (newInterval >= 20 && newInterval <= 1000) {
      ...
      response += " to " + value + "ms";
      Serial.println("  刷新率已设置为: " + value + "ms");
    } else {
      response = "错误：刷新率超出范围 (20-1000ms)";
      Serial.println("  错误：刷新率设置超出范围");
    }
  } 
  else if (command == "strobe_detect") {
    startStrobeDetection();
    response = "频闪检测启动";
    Serial.println("  频闪检测模式已启动");
  }
...
void showSystemLog() {
  ...
  logPage += "<p>频闪检测: " + String(strobeDetectionActive ? "进行中" : "未激活") + "</p>";
...
// 数据下载功能
void downloadData() {
  Serial.print("[" + String(millis()) + "] 客户端请求数据下载 - ");
  Serial.println("客户端IP: " + server.client().remoteIP().toString());
  
  String csvData = "时间戳,ADC值,电压(V)\n";
  int startIndex = historyFull ? historyIndex : 0;
  int dataCount = historyFull ? HISTORY_SIZE : historyIndex;
  
  for (int i = 0; i < dataCount; i++) {
    int idx = (startIndex + i) % HISTORY_SIZE;
    csvData += String(timeHistory[idx]) + ",";
    csvData += String(lightHistory[idx]) + ",";
    csvData += String((lightHistory[idx] * 3.3) / 4095.0, 3) + "\n";
  }
  
  server.send(200, "text/csv", csvData);
  Serial.println("  CSV数据已发送，包含 " + String(dataCount) + " 个数据点");
}
...
void updateLightHistory(int value, unsigned long timestamp) {
  lightHistory[historyIndex] = value;
  timeHistory[historyIndex] = timestamp;
  historyIndex = (historyIndex + 1) % HISTORY_SIZE;
  if (historyIndex == 0) historyFull = true;
}

void handleStrobeDetection(int value, unsigned long currentTime) {
  if (strobeIndex < STROBE_SAMPLES) {
    strobeData[strobeIndex++] = value;
  }
  
  if (currentTime - strobeDetectionStartTime >= strobeDetectionDuration) {
    strobeDetectionActive = false;
    strobeDataReady = true;
    
    String result = analyzeStrobeData();
    Serial.println("频闪检测完成: " + result);
    
    updateInterval = 100;
  }
}

String analyzeStrobeData() {
  // 简化的频闪分析逻辑
  float sum = 0;
  for (int i = 0; i < strobeIndex; i++) sum += strobeData[i];
  float mean = sum / (float)strobeIndex;
  
  float variance = 0;
  for (int i = 0; i < strobeIndex; i++) {
    variance += pow(strobeData[i] - mean, 2);
  }
  float stdDev = sqrt(variance / strobeIndex);
...
void startStrobeDetection() {
  strobeDetectionActive = true;
  strobeDetectionStartTime = millis();
  strobeIndex = 0;
  strobeDataReady = false;
  updateInterval = 50;
}
...
void updateOLEDDisplay(int value) {
  ...
  display.print(systemPaused ? "PAUSED" : "RUNNING");
